...
function proxyToGoogleAnalytics(event) {
    ...
    const params =
        event.httpMethod.toUpperCase() === "GET" ? event.queryStringParameters : querystring.parse(event.body)
...
exports.handler = function(event, context, callback) {
    ...
    const isOriginallowlisted =
        originallowlist.indexOf(origin) >= 0 || origin.endsWith("-upbeat-shirley-608546.netlify.app") >= 0
    ...
    const headers = {
        ...
        "Access-Control-Allow-Origin": isOriginallowlisted ? origin : originallowlist[0],
